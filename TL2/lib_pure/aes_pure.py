...
def aes256_encrypt(key, plaintext):
    """Encrypt plaintext with AES256, PKCS7 padding, ECB mode."""
    assert len(key) == 32, "Key must be 32 bytes"
    aes = AES(key)
    padded = pad(plaintext)
    out = b""
    for i in range(0, len(padded), 16):
        out += aes.encrypt_block(padded[i:i+16])
    return out

def aes256_decrypt(key, ciphertext):
    """Decrypt ciphertext with AES256, PKCS7 padding, ECB mode."""
    assert len(key) == 32, "Key must be 32 bytes"
    assert len(ciphertext) % 16 == 0, "Ciphertext must be multiple of 16"
    aes = AES(key)
    out = b""
    for i in range(0, len(ciphertext), 16):
        out += aes.decrypt_block(ciphertext[i:i+16])
    return unpad(out)
